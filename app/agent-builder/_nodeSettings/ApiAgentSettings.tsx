...
function ApiAgentSettings({ selectedNode, updateFormData }: any) {
  ...
  const [formData, setFormData] = useState({
    ...
    includeApiKey: true,
    bodyParams: '',
    paramKey: 'q' // ✅ dynamic param key
  })

  // Load existing data when node selected
  useEffect(() => {
    if (selectedNode?.data?.settings) {
      setFormData({
        ...
        paramKey: selectedNode.data.settings.paramKey || 'q'
      })
    }
  }, [selectedNode])
  ...
  const onSave = () => {
    ...
    // ✅ Remove query params from URL (clean base URL)
    const cleanUrl = formData.url.split('?')[0]

    const finalData = {
      ...formData,
      url: cleanUrl,

      // ✅ Dynamic parameters (IMPORTANT)
      parameters: {
        [formData.paramKey]: `{${formData.paramKey}}`
      }
    }

    console.log("✅ SAVING API CONFIG:", finalData)
    ...
    <div>
      ...
      <div className='mt-3 space-y-1'>
        ...
        <p className="text-xs text-gray-500">
          ⚠️ Do NOT include query params (like ?q=)
        </p>
      </div>

      {/* Param Key (🔥 IMPORTANT FEATURE) */}
      <div className='mt-3 space-y-1'>
        <Label>Query Parameter Key</Label>

        <Input
          placeholder="e.g. q / city / location / id"
          value={formData.paramKey}
          onChange={(e) => handleChange('paramKey', e.target.value)}
        />

        <p className="text-xs text-gray-500">
          Example: Weather API → q
        </p>
      </div>
      ...
      {formData.method === "POST" && (
        <div className='mt-3 space-y-1'>
          ...
          <h2 className='text-sm p-1 flex gap-2 items-center'>
            Add Params <FileJson className='h-3 w-3' />
          </h2>
        </div>
      )}
